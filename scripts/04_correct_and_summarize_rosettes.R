...
# =============================================================================
# 04_summarize_rosette_cells.R
#
# Step 4 summary script for rosette/cell analysis.
#
# Supports:
#   1. Normal mode:
#        --input points directly at a Step 3 output folder.
#
#   2. Project-level mode:
#        --input points at a project folder organized like:
#
#        Project/sample/replicate/image/<cell table>
#
#      Example:
#        results/rosette_project/SampleA/replicate_01/Snap_001/all_cells.tsv
#
# Default behavior:
#   - Final summaries use accepted cells only.
#   - Raw combined table still preserves rejected cells.
#   - Rosettes with fewer than --min_cells_per_rosette accepted cells are demoted
#     into single/unassigned counts when single-cell correction is enabled.
#
# =============================================================================
...
SCRIPT_VERSION <- "04_summary_v03_project_level_accepted_cells_default"
...
option_list <- list(
  make_option(
    ...
    type = "character",
    default = NULL,
    help = "Input directory. Either a Step 3 folder or a project-level folder."
  ),
  ...
  make_option(
    ...
    type = "character",
    default = NULL,
    help = "Output directory for Step 4 summary tables."
  ),
  ...
  make_option(
    ...
    default = FALSE,
    help = paste(
      "If TRUE, interpret input paths as project/sample/replicate/image.",
      "Default: FALSE"
    )
  ),
  ...
  make_option(
    ...
    default = 1,
    help = paste(
      "In project-level mode, which path part under --input is sample_id.",
      "Default: 1"
    )
  ),
  ...
  make_option(
    ...
    default = 2,
    help = paste(
      "In project-level mode, which path part under --input is replicate_id.",
      "Default: 2"
    )
  ),
  ...
  make_option(
    ...
    default = 3,
    help = paste(
      "In project-level mode, which path part under --input is image_id.",
      "Default: 3"
    )
  ),
  ...
  make_option(
    ...
    default = "",
    help = paste(
      "Optional exact cell table filename to search for.",
      "If blank, common names are tried automatically."
    )
  ),
  ...
  make_option(
    ...
    default = TRUE,
    help = paste(
      "Use only accepted cells for final summaries if accepted_cell/accepted_cells",
      "column is present. Default: TRUE"
    )
  ),

  make_option(
    c("--single_cell_rosette_correction"),
    type = "logical",
    default = TRUE,
    help = paste(
      "If TRUE, rosettes represented by fewer than --min_cells_per_rosette",
      "accepted cells are dropped from rosette counts and counted as",
      "single/unassigned cells. Default: TRUE"
    )
  ),

  make_option(
    c("--min_cells_per_rosette"),
    type = "integer",
    default = 2,
  ...
  make_option(
    ...
    default = "",
    help = paste(
      "Optional sample column name for non-project mode.",
      "In project-level mode, folder-derived sample_id is used by default."
    )
  ),
...
infer_ids_from_path <- function(source_file, input_root, project_level,
  ...
  if (isTRUE(project_level)) {
    ...
    if (is.na(replicate_id) || replicate_id == "") {
      replicate_id <- "unknown_replicate"
    }

    if (is.na(image_id) || image_id == "") {
      image_id <- tools::file_path_sans_ext(basename(source_file))
    }
  ...
  # Non-project mode:
  # Try to use the first folder under input as image/sample label.
  fallback <- if (length(parts) >= 1) {
    parts[[1]]
  } else {
    tools::file_path_sans_ext(basename(source_file))
  }
...
find_cell_tables <- function(input_dir, exact_name = "", recursive = TRUE) {
  ...
  if (!is.null(exact_name) && nzchar(exact_name)) {
    ...
    # Avoid accidentally reading prior Step 4 summaries.
    all_tsv <- all_tsv[
      !grepl("/04_summary/", all_tsv, fixed = TRUE) &
        !grepl("/summary/", all_tsv, ignore.case = TRUE) &
...
standardize_cell_table <- function(dt, source_file, input_root, opt) {
  ...
  if (nzchar(opt$cell_col)) {
    if (!opt$cell_col %in% names(dt)) {
      stop("Requested --cell_col not found: ", opt$cell_col, call. = FALSE)
    }
  ...
  # Make a globally unique cell UID because cell_id often repeats across images.
  ...
  if (nzchar(opt$rosette_col)) {
    if (!opt$rosette_col %in% names(dt)) {
      stop("Requested --rosette_col not found: ", opt$rosette_col, call. = FALSE)
    }
  ...
  dt[, rosette_uid := ifelse(
    is.na(rosette_id),
    NA_character_,
    paste(source_file, rosette_id, sep = "__")
  )]
  ...
  if (nzchar(opt$area_col)) {
    if (!opt$area_col %in% names(dt)) {
      stop("Requested --area_col not found: ", opt$area_col, call. = FALSE)
    }
  ...
  accepted_col <- pick_first_existing_col(
    dt,
    c("accepted_cell", "accepted_cells", "cell_accepted", "accepted", "pass_cell", "cell_pass"),
    required = FALSE,
...
summarize_level <- function(level_name,
  ...
  if (nrow(rosette_summary) > 0) {
    ...
    rosette_counts <- copy(all_groups)
    rosette_counts[, n_rosettes := integer()]
    rosette_counts[, total_cells_in_rosettes := integer()]
    rosette_counts[, mean_cells_per_rosette := numeric()]
    rosette_counts[, median_cells_per_rosette := numeric()]
    rosette_counts <- rosette_counts[0]
  }
  ...
  if (nrow(cells_single_summary_input) > 0) {
    ...
    single_counts <- copy(all_groups)
    single_counts[, n_single_or_unassigned_cells := integer()]
    single_counts[, n_single_or_unassigned_rows := integer()]
    single_counts <- single_counts[0]
  }
  ...
  out[
    ,
    total_accepted_cells_after_correction :=
      as.integer(total_cells_in_rosettes + n_single_or_unassigned_cells)
  ]
...
fwrite(
  cells_all,
  file.path(opt$out, "all_cells_combined.tsv"),
  sep = "\t"
)
...
if (isTRUE(opt$accepted_only)) {
  ...
  cells_summary_input <- cells_all[
    !is.na(accepted_cell_for_summary) &
      accepted_cell_for_summary == TRUE
  ]
...
fwrite(
  cells_summary_input,
  file.path(opt$out, "all_cells_summary_input.accepted_only.tsv"),
  sep = "\t"
)
...
fwrite(
  cells_rosette_summary_input,
  file.path(opt$out, "cells_used_for_rosette_summary.tsv"),
  sep = "\t"
)

fwrite(
  cells_single_summary_input,
  file.path(opt$out, "cells_counted_as_single_or_unassigned.tsv"),
  sep = "\t"
)
...
fwrite(
  rosette_summary,
  file.path(opt$out, "rosette_summary.tsv"),
  sep = "\t"
)
...
fwrite(
  image_summary,
  file.path(opt$out, "image_summary.tsv"),
  sep = "\t"
)

fwrite(
  replicate_summary,
  file.path(opt$out, "replicate_summary.tsv"),
  sep = "\t"
)

fwrite(
  sample_summary,
  file.path(opt$out, "sample_summary.tsv"),
  sep = "\t"
)
...
global_summary <- data.table(
  ...
  raw_rejected_cell_rows = as.integer(
    sum(cells_all$accepted_cell_for_summary == FALSE | is.na(cells_all$accepted_cell_for_summary), na.rm = TRUE)
  ),
  ...
  total_accepted_cells_after_correction = as.integer(
    sum(rosette_summary$n_cells, na.rm = TRUE) +
      safe_uniqueN(cells_single_summary_input$cell_uid)
  )
)

fwrite(
  global_summary,
  file.path(opt$out, "global_summary.tsv"),
  sep = "\t"
)

# Also write alias for project mode.
fwrite(
  global_summary,
  file.path(opt$out, "project_summary.tsv"),
  sep = "\t"
)
...
report_lines <- c(
  ...
  "",
  paste0("n_rosettes: ", global_summary$n_rosettes),
  paste0("total_cells_in_rosettes: ", global_summary$total_cells_in_rosettes),
  paste0("n_single_or_unassigned_cells: ", global_summary$n_single_or_unassigned_cells),
